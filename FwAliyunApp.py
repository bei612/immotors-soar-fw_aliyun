...
class FwAliyunApp:
    ...
    def auto_unblock_task(self, addr, direction=None):
        ...
        # list_success_addrs 为解封成功的ip地址字典的列表
        # list_failed_addrs 为解封失败的ip地址字典的列表
        list_success_addrs = []
        list_failed_addrs = []
        list_remain_addrs = list(addrs)
        ...
        # 主处理循环
        # list_remain_addrs 列表为待处理地址列表，会在执行过程中被消费直到列表为空，退出while循环
        while list_remain_addrs:
            len_list_remain_addrs = len(list_remain_addrs)
            msg = {
                "addr": f"{list_remain_addrs}",
            ...
            for describe_address_book_grouptype, list_addrs_groups in addrs_groups.items():
                ...
                for addrgrp in res_describe_address_book['body']['Acls']:
                    # 如果当前类型中没有剩余的待解封的IP，则跳出当前类型循环
                    if not list_addrs_groups:
                        break
                    # 如果当前地址组名称不包含查询前缀，则跳过当前地址组
                    ...
                    # 查找当前地址组中需要移除的IP
                    list_matched_addrs = []
                    for addr_to_remove in list_addrs_groups[:]:
                        if addr_to_remove in addrgrp_addresslist:
                            list_matched_addrs.append(addr_to_remove)
                    
                    # 如果找到匹配的IP，进行移除操作
                    if list_matched_addrs:
                        # 构建新的地址列表（移除匹配的IP）
                        new_addresslist = [addr for addr in addrgrp_addresslist if addr not in list_matched_addrs]
                        data_addrgrp_addresslist = ','.join(new_addresslist) if new_addresslist else utils.get_config('add_address_book.addresslist')
                        ...
                        if isinstance(res_modify_address_book, dict) and res_modify_address_book.get('statusCode') == 200:
                            for matched_addr in list_matched_addrs:
                                msg = {
                                    "addr": f"{matched_addr}",
                                    "groupname": f"{addrgrp_groupname}",
                                    "groupuuid": f"{addrgrp_groupuuid}",
                        ...
                        # 从待处理列表中移除已处理的地址
                        list_addrs_groups = [x for x in list_addrs_groups if x not in list_matched_addrs]
                        list_remain_addrs = [x for x in list_remain_addrs if x not in list_matched_addrs]
                        
                        # 找到匹配后跳出当前地址簿循环，继续处理下一个地址簿
                        break
            
            # while主循环保护机制
            # 如果本轮循环没有处理任何IP，则将剩余IP标记为未找到并退出循环
            if len(list_remain_addrs) == len_list_remain_addrs:
                consecutive_failures += 1
                if consecutive_failures >= 1:  # 改为1次失败就退出，避免重复处理
                    # 将剩余IP标记为未找到
